...
def CSQ_init(n_class, bit):
    ...
    hash_center = h_2k[:n_class]   # 如果n_class大于h_2k.shape[0]，hash_center = h_2k[:]
...
def get_hash_centers(bit, n_class):
    ...
    if not os.path.exists('./hash_centers'):
        os.mkdir('./hash_centers')
    np.save(f'./hash_centers/num_classes-{n_class}-bit-{bit}.npy', best_B)
    return best_B
