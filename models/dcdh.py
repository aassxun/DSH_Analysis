...
class SW_layer(nn.Module):
    ...
    def forward(self, input, label):  # input:[N, bits], label:[N,]
        ...
        max_likehood_loss = 0.5 * ((input - self.mu[:, label].transpose(1, 0)) ** 2 / (torch.max(
            torch.Tensor([0]).cuda() + 1e-9, self.sigma[:, label].transpose(1, 0)))).sum(dim=1)
        max_likehood_loss -= torch.log(torch.max(torch.Tensor(
            [0]).cuda() + 1e-9, (self.sigma[:, label].prod(dim=0))**(-0.5)))
        ...
        pro_res = (((x - self.mu)**2) / (torch.max(self.sigma,
                                                   torch.Tensor([0]).cuda() + 1e-9))).sum(dim=1)  # distance [N, cls]
        pro_res = (-0.5) * pro_res
...
class dcdh_resnet(nn.Module):
    def __init__(self, code_length, pretrained):
        super().__init__()
...
def dcdh(code_length,pretrained):
    model = dcdh_resnet(code_length, pretrained)
    return model
