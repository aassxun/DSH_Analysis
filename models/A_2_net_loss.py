...
class A_2_net_Loss(nn.Module):
    ...
    def forward(self, F, B, S, omega, dret, all_f, deep_S, inputs, iter):
        ...
        if self.finetune:
            ...
            loss = hash_loss + quantization_loss + reconstruction_loss + decorrelation_loss #+ gan_loss
        else:
            with torch.no_grad():
                reconstruction_loss = self.MSE_loss(dret, all_f)
            if iter >= 35:
                loss = hash_loss + quantization_loss + decorrelation_loss + reconstruction_loss #+ gan_loss
            else:
