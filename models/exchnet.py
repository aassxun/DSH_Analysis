...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
           ...
           'wide_resnet50_2', 'wide_resnet101_2', 'exchnet_backbone']
...
def exchnet_backbone(pretrained=True, progress=True, **kwargs):
...
def exchnet_refine(is_local=True, pretrained=True, progress=True, **kwargs):
...
def exchnet_attention(att_size=4, pretrained=False, progress=True, **kwargs):
...
class ExchNet(nn.Module):
    def __init__(self, code_length=48, num_classes=200, att_size=4, feat_size=2048, device='cpu', pretrained=True):
        ...
        self.refine_local = exchnet_refine(pretrained=pretrained)
        self.attention = exchnet_attention(att_size)
    ...
    def forward(self, x, targets):
        ...
        # local_f_sum = local_f.sum(dim=2).view(batch_size, att_size, -1)
        # local_f_sum_max = local_f_sum.max(dim=2).values.view(batch_size, att_size, 1)
        local_f_mean = local_f.mean(dim=2).view(batch_size, att_size, -1)
