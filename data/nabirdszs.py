...
def load_data(root, batch_size, num_workers,zero_shot_classes = 27):
    NABirds.init(root,zero_shot_classes = zero_shot_classes)
    ...
    query4zero_shot_dataset = NABirds(root, 'query4zero_shot', query_transform())
    database4zero_shot_dataset = NABirds(root, 'database4zero_shot', query_transform())
...
class NABirds(VisionDataset):
    ...
    def __init__(self, root, mode, transform=None, target_transform=None):
        ...
        if mode == 'train':
            ...
            self.data = NABirds.QUERY_DATA4ZERO_SHOT
            self.targets = NABirds.QUERY_TARGETS4ZERO_SHOT
        elif mode == 'database4zero_shot':
            self.data = NABirds.DATABASE_DATA4ZERO_SHOT
            self.targets = NABirds.DATABASE_TARGETS4ZERO_SHOT
        else:
            raise ValueError(r'Invalid arguments: mode, can\'t load dataset!')


    @staticmethod
    def init(root,zero_shot_classes):
        ...
        class4train = class_num  - zero_shot_classes
        ...
        NABirds.TRAIN_TARGETS = encode_onehot((train_data['target']).tolist(), class4train)
        ...
        NABirds.QUERY_DATA4ZERO_SHOT = query_data4zero_shot['filepath'].to_numpy()
        NABirds.QUERY_TARGETS4ZERO_SHOT = encode_onehot((query_data4zero_shot['target']).tolist(), class_num)

        NABirds.DATABASE_DATA4ZERO_SHOT = database_4zero_shot['filepath'].to_numpy()
        NABirds.DATABASE_TARGETS4ZERO_SHOT = encode_onehot((database_4zero_shot['target']).tolist(), class_num)
...
if __name__ == '__main__':
    load_data('/2T/dataset/nabirds', 16, 2, 27)
    # train_dataset = NABirds('/2T/dataset/nabirds', train=True)
    # test_dataset = NABirds('/2T/dataset/nabirds', train=False)
