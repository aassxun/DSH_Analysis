...
def load_data(root, batch_size, num_workers,zero_shot_classes):
    Aircraft.init(root,zero_shot_classes)
    ...
    query4zero_shot_dataset = Aircraft(root, 'query4zero_shot', query_transform())
    database4zero_shot_dataset = Aircraft(root, 'database4zero_shot', query_transform())
    print(len(query_dataset))
    print(len(train_dataset))
...
class Aircraft(Dataset):
    ...
    def __init__(self, root, mode, transform=None, loader=default_loader):
        ...
        if mode == 'train':
            ...
            self.data = Aircraft.QUERY_DATA4ZERO_SHOT
            self.targets = Aircraft.QUERY_TARGETS4ZERO_SHOT
        elif mode == 'database4zero_shot':
            self.data = Aircraft.DATABASE_DATA4ZERO_SHOT
            self.targets = Aircraft.DATABASE_TARGETS4ZERO_SHOT
        else:
    ...
    def init(root,zero_shot_classes = 5):
        ...
        Aircraft.TRAIN_TARGETS = encode_onehot((train_data['target'] - 1).tolist(), class4train)
        ...
        Aircraft.QUERY_DATA4ZERO_SHOT = query_data4zero_shot['filepath'].to_numpy()
        Aircraft.QUERY_TARGETS4ZERO_SHOT = encode_onehot((query_data4zero_shot['target'] -1 ).tolist(), 100)

        Aircraft.DATABASE_DATA4ZERO_SHOT = database_4zero_shot['filepath'].to_numpy()
        Aircraft.DATABASE_TARGETS4ZERO_SHOT = encode_onehot((database_4zero_shot['target'] - 1).tolist(), 100)
